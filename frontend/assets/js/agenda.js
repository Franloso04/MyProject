...
async function loadSessions() {
    ...
    try {
        ...
        agendaContainer.innerHTML =
            sesiones.length === 0
                ? '<p class="text-center py-10 opacity-50 italic">No hay sesiones.</p>'
                : "";
        ...
        sesiones.forEach((ses) => {
            ...
            // ✅ CORRECCIÓN: definimos hFin
            const end = ses.hora_fin ? new Date(ses.hora_fin) : null;
            const hFin = end ? end.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "--:--";
            ...
            agendaContainer.insertAdjacentHTML(
                "beforeend",
                `
...
if (createSpeakerForm) {
    ...
    async function saveEventSettings() {
        const selectedLangs = Array.from(document.querySelectorAll('input[name="lang"]:checked')).map(
            (el) => el.value
        );
